...
// (undefined/null) 미설정으로 허용하고, 1 이상의 safe integer일 때만 유효합니다.
// 그 외(빈 문자열 ""을 포함해 문자열/소수/0/음수/boolean/2^53 이상의 정밀도를
// 잃은 큰 수 등)는 모두 거부합니다. Number.isInteger() 대신 Number.isSafeInteger()를
// 쓰는 이유: priority는 순서를 정확히 결정하는 값인데, JS는 2^53 이상의 매우 큰
// 수도 정수로 판정할 수 있어(정밀도 손실로 서로 다른 두 값이 같은 값으로 취급될
// 수 있음) 그런 값을 허용하면 "정확한 순서"라는 필드의 목적이 깨질 수 있습니다.
